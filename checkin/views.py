...
def export_pdf(request, company_name='ALL'): 
    """
    ...
    if company_name and company_name != 'ALL':
        # ค้นหาแบบตรงตัวในฟิลด์บันทึกประวัติกองร้อย (เช่น 'ร้อย.1', 'ร้อย.2' หรือ '1', '2')
        users_queryset = users_queryset.filter(profile__company=company_name)
        
        # ปรับการแสดงผลชื่อหัวข้อรายงานให้สวยงามตามหน้ากากเว็บ
    ...
    # 3. 🛠️ แก้ไข Logic แยกชุดข้อมูลกำลังพลเป็น 3 กลุ่มตามสถานะจริงใน DB ของคุณ
    
    # กลุ่มที่ 1: อยู่ระหว่างการลา (PENDING)
    pending_users = users_queryset.filter(profile__return_status='PENDING').order_by('username')
    
    # กลุ่มที่ 2: กลับรายงานตัวสาย (LATE)
    late_users = users_queryset.filter(profile__return_status='LATE').order_by('username')
    
    # กลุ่มที่ 3: กลับเข้ารายงานตัวปกติสำเร็จ (เปลี่ยนจาก ON_TIME เป็น SUCCESS ให้ตรงกับระบบเช็กอินสแกนหน้าของคุณ)
    on_time_users = users_queryset.filter(profile__return_status='SUCCESS').order_by('username')
